...
class TestComplexityScore:
    def test_simple_sentence(self):
        score = complexity_score_zh("你好")
        assert 0 < score < 20  # Simple sentence should have low complexity

    def test_complex_sentence(self):
        score = complexity_score_zh("这是一个非常复杂的句子，包含很多不同的汉字")
        assert score > 50  # Complex sentence should have high complexity

    def test_single_char(self):
        score = complexity_score_zh("好")
        assert score < 15  # Single char is very simple

    def test_empty_returns_zero(self):
        assert complexity_score_zh("") == 0.0
        assert complexity_score_zh("hello") == 0.0  # No Chinese chars
...
class TestGetWordRank:
    ...
    def test_known_word(self, freq_data):
        assert get_word_rank_zh("你", freq_data) == 2

    def test_unknown_word_fallback_to_chars(self, freq_data):
        # 你好 not in word_freq, should average char ranks (2+3)/2 = 2.5 -> 2
        rank = get_word_rank_zh("你好", freq_data)
        assert rank == 2  # Average of 2 and 3, truncated

    def test_unknown_chars_max_rank(self, freq_data):
        rank = get_word_rank_zh("稀罕", freq_data, max_rank=50000)
        assert rank == 50000


class TestCharSimilarity:
    def test_identical(self):
        assert char_similarity_zh("你好", "你好") == 1.0

    def test_no_overlap(self):
        assert char_similarity_zh("你好", "再见") == 0.0

    def test_partial_overlap(self):
        sim = char_similarity_zh("你好", "你们好")
        assert 0 < sim < 1

    def test_empty_returns_zero(self):
        assert char_similarity_zh("", "你好") == 0.0
        assert char_similarity_zh("你好", "") == 0.0
