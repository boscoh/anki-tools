...
POLYPHONIC_SKIP = {
    '音樂', '銀行', '乾淨', '睡覺', '和',
    '長', '行', '樂', '發', '數', '教', '覺', '著', '了', '地', '的',
}

NEUTRAL_TONE_PATTERNS = [
    '明白', '喜欢', '这里', '那里', '哪里', '回来', '起来', '出来', '进来',
    '先生', '漂亮', '便宜', '清楚', '告诉', '知道', '认识', '觉得', '意思',
    '东西', '时候', '朋友', '客气', '舒服', '麻烦', '厉害', '暖和',
    '小姐', '謝謝', '記得', '爸爸', '媽媽', '姐姐', '妹妹', '哥哥', '弟弟',
    '太太', '奶奶', '爷爷', '姑姑', '叔叔', '阿姨',
]
...
def is_neutral_tone_word(hanzi):
...
def needs_formatting(csv_pinyin):
    """Check if pinyin needs lowercase or punctuation removal."""
    csv_letters = re.sub(r'[^a-zA-ZāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜĀÁǍÀĒÉĚÈĪÍǏÌŌÓǑÒŪÚǓÙǕǗǙǛ\s]', '', csv_pinyin)
    return csv_letters != csv_letters.lower()


def check_pinyin(hanzi, csv_pinyin):
    ...
    csv_clean = re.sub(r'[^a-zA-ZāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜĀÁǍÀĒÉĚÈĪÍǏÌŌÓǑÒŪÚǓÙǕǗǙǛ\s]', '', csv_pinyin)
    words = csv_clean.split()
    csv_syllables = 0
    for word in words:
        csv_syllables += len(re.findall(r'[aeiouāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜAEIOUĀÁǍÀĒÉĚÈĪÍǏÌŌÓǑÒŪÚǓÙǕǗǙǛ]+', word, re.I))
    
    if hanzi_chars >= 2 and csv_syllables > 0:
        if csv_syllables < hanzi_chars:
            return expected, f'missing syllables: {csv_syllables} for {hanzi_chars} chars'
    ...
    skip_tone_check = (
        any(char in hanzi for char in POLYPHONIC_SKIP) or
        '不' in hanzi or '一' in hanzi or
        is_neutral_tone_word(hanzi) or
        hanzi.endswith(('阿', '呀', '嘛', '吧', '呢', '啊')) or
        '得' in hanzi
    )
    ...
    if skip_tone_check:
        ...
        if hanzi_chars >= 2 and len(csv_words) < hanzi_chars:
            return expected, f'needs syllable separation: {len(csv_words)} words for {hanzi_chars} chars'
        if needs_formatting(csv_pinyin):
...
def fix_pinyin(
    ...
    *,
    ...
    Args:
        apkg_path: Input .apkg file to process
        output: Output .apkg file (default: input_fixed.apkg)
        csv_output: Output CSV report (default: input_cards.csv)
        verbose: Print all corrections
    """
    ...
    with AnkiPackage(str(apkg_path)) as pkg:
        ...
        for i, card in enumerate(cards):
            ...
            chinese = (fields.get("Chinese") or fields.get("Hanzi") or 
                       fields.get("中文") or fields.get("Sentence") or "")
            pinyin_val = (fields.get("Pinyin") or fields.get("拼音") or 
                          fields.get("Sentence (Latin)") or "")
            meaning = (fields.get("Meaning") or fields.get("English") or 
                       fields.get("Sentence (Translation)") or "")
    ...
    if csv_output and str(csv_output) != "/dev/null":
        with open(csv_output, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=["index", "hanzi", "pinyin", "meaning", "correction", "reason"])
            writer.writeheader()
    ...
    if verbose and corrections:
        print("\nCorrections applied:")
        for i, corr in corrections.items():
            print(f"  {i}: {corr['hanzi']:<15} '{corr['old_pinyin']}' -> '{corr['new_pinyin']}'")
